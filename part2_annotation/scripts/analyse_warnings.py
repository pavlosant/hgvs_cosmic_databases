...
def part2_analysis(samples, dias_folder, output_file):
    ...
    samples_with_problem_line=find_problem_line_from_dias_files(samples_with_dias_info)
...
def part2_print_to_file(output_file,samples):
    with open(output_file,'w') as outf:
        ...
        for s in samples:
            for w in s.warnings:
                if w.warning_type == INPUT_WARNING:
                    outf.write(f"{s.sampleID}\t{w.warning}\t{w.problem_line}\t{s.vep_out_path}\t{s.dias_path}\n")
...
def main(args):
    ...
    Part 1
    '''
    samples_with_warnings=part1_analysis(args.vep_folder, args.warnings_output_file1)
    
    
    '''
    Part 2
    '''
    part2_analysis(samples_with_warnings, args.dias_folder, args.warnings_output_file2)
